...
# Part 9 — Delivered releases (v0.1.0 → v0.9.0)

> **Why this Part exists.** The specification above says what the system should
> do. It did not say what it *does*, and the gap had grown to roughly fifty
> commits: the last documentation update predates every versioned release. This
> Part closes that gap and is the place to extend on each future bump.
>
> The rule that produced these numbers is in `CLAUDE.md`: every batch of work
> that is not purely cosmetic raises the version before it merges — minor for
> features, patch for fixes — and the number lives in exactly two files
> (`src/hermes_assistant/__init__.py` and `pyproject.toml`), which a test keeps
> in agreement. The dashboard's topbar chip reads it through `/api/health`, so
> the chip answers "am I looking at the build I just shipped?".

## 9.1 Release history

| Version | Shipped | Kind |
|---|---|---|
| 0.1.0 | Version chip in the dashboard topbar; the Vue dashboard, chat with model switching, SSE streaming, Risk Registry screen, per-tool Copilot prompts, hardened import pipeline | baseline |
| 0.2.0 | Pendenzen → **Todo** rename across every surface; reorderable sidebar; create Todo / work package / project by hand; Beschlussliste and detailed Ablaufplan import with a Gantt; the cross-source sweep of every dated obligation; person filter, inline owner edit, timeline zoom | feature |
| 0.3.0 | **Delete with undo** for todos, work packages and projects (soft delete + trash directory); **Ollama failover on every request**, not only chat routing; Todo and Termine & Fristen merged into one screen; stale static assets fixed | feature |
| 0.4.0 | **Microsoft 365 Copilot POC** — Retrieval + Chat clients, device-code auth, off by default | feature |
| 0.4.1 | The JSON import dialog closes itself after a clean import | fix |
| 0.5.0 | Timeline rework: sync to today, real time scales (Jahr…Tag), multi-select owner filter, timeline as landing screen, reorderable pages | feature |
| 0.5.1 | The timeline opens from today rather than three months of finished work | fix |
| 0.5.2 | The axis no longer reaches into the past; the track is drag-pannable | fix |
| 0.5.3 | Task names pinned while panning; the axis starts on today exactly | fix |
| 0.6.0 | Screen renamed **Planung**, opens on the timeline; **Frist presets** (Ohne · Heute · Diese Woche · Datum) when creating a todo; dated todos appear on the timeline | feature |
| 0.7.0 | Twelve restated counts removed from two screens; **smart capture** — one sentence into the create form via the local model | feature |
| 0.8.0 | **Every task is a bar**, spanning the time it has left | feature |
| 0.8.1 | "Task not found" when deleting a swept plan item | fix |
| 0.9.0 | **Copilot POC page** in the sidebar, marked temporary | feature |

## 9.2 What each release changed, and why

### 0.2.0 — one vocabulary, and a dashboard you can write to

"Pendenz" and "Todo" named the same thing in different places. The rename went
through the Vue dashboard, the second HTML renderer, the TUI, the CLI and the
chat placeholder — the first pass missed three of those, which is why 0.2.0
took two commits.

The dashboard also stopped being read-only: todos, work packages and projects
can be created by hand, and the sidebar order is the user's and persists.

The import side gained the **cross-source sweep** — one prompt that returns
every dated obligation at any altitude, replacing several narrower exports —
plus the Beschlussliste and the detailed Ablaufplan with a Gantt over them.

### 0.3.0 — recoverable deletes, and a model that fails over

Nothing on the dashboard could be removed. Delete now exists for todos, work
packages and projects, and all three are recoverable: tasks are soft-deleted
(`deleted_at`, restored by id, parent-first), a project directory is moved to a
trash directory beside `projects/` rather than unlinked, and both offer Undo in
the toast.

Model failover moved **into the Ollama client**, so every call site inherits it
— the router, intake, planner, critic, red team and meeting extraction — rather
than only the chat router that had it.

Two screens became one: Todo and Termine & Fristen were never two kinds of
thing, only one question split by whether an item happened to carry a date.

Also in 0.3.0: `Cache-Control: no-cache` on `/static/` and `?v=<version>` on
asset URLs. Starlette's `StaticFiles` sends `ETag` and `Last-Modified` but no
`Cache-Control`, so browsers fall back to heuristic caching and a shipped UI
change simply does not appear, with no error to explain it.

### 0.4.0 — the Microsoft 365 exception

HERMES is otherwise fully local. `src/hermes_assistant/m365/` is the one
deliberate exception and stays **off** unless `HERMES_M365_ENABLED` is set and
a person signs in. Two APIs: `retrieval` returns permission-trimmed extracts
(Microsoft does the chunking and ranking, so no local index and no copy of
tenant content), and `chat` is a grounded conversation that answers with text
only — right for state capture, still wrong for document generation.

Delegated permissions only, by force of the API: there is no application
identity that can read tenant content this way, so HERMES cannot widen anyone's
access. Sign-in is device-code; the token cache holds refresh tokens, so it is
written `0600` under the data directory.

`tests/test_no_cloud.py` was narrowed rather than weakened: Microsoft endpoints
are permitted **inside `m365/` only**, and a test asserts the integration ships
disabled.

### 0.5.0 – 0.5.3 — making the timeline tell the truth about "now"

Four releases on one screen, each fixing something the previous one exposed:

1. **Today-sync and real scales.** The axis had no relationship to the current
   date and "zoom" changed nothing meaningful. Scales are now Jahr / Quartal /
   Monat / Woche / Tag with fixed px-per-day, so a day is the same width
   whatever the plan's length.
2. **Opening from today.** The default window was three months of mostly
   finished work.
3. **The axis reaching into the past.** Selecting rows and sizing the axis are
   two jobs, and only the first respected the window: one overdue item that
   *started* in July dragged the whole ruler back two months. The axis now
   clamps to the window; a bar that began earlier is clipped at the edge.
4. **Pinned names, and today as the hard left edge.** Panning took the task
   names with it, leaving anonymous coloured stripes; the label column is now
   sticky. And the axis reached back to the oldest overdue deadline so an
   already-ended item had somewhere to draw — that bought one bar its lane at
   the cost of the ruler, so the past is off the axis entirely and those items
   are drawn as a stub at the edge.

Drag-to-pan arrived in 0.5.2 (pointer events with `setPointerCapture`, so pen
and touch work the same way).

### 0.6.0 — Planung, and asking how soon rather than which day

The merged screen is **Planung** — a name covering all three lenses rather than
repeating the one shown first — and it opens on the timeline. The route key
`work` and the `#/pendenzen` and `#/plan` aliases are unchanged, so bookmarks
and stored sidebar orders keep working.

Creating a todo asks **Ohne · Heute · Diese Woche · Datum …** and shows the date
the choice resolved to. "Ohne" stays the default: inventing a deadline nobody
set would put a false bar on the timeline and, a week later, a false overdue
count. "Diese Woche" is the Friday of the working week — a deadline landing on
a Sunday is one nobody will meet. The preset resolves at submit, not at click,
so a dialog left open past midnight does not post yesterday's "Heute".

This also closed a gap the landing-page change exposed: the Gantt reads
`data.ablaufplan`, which holds only what a sweep imported, so a todo you had
just given a deadline to would not appear on the screen you would go looking
for it. Dated todos are now folded into the timeline lens as `kind: 'termin'`.

### 0.7.0 — cutting what the screen already said, and smart capture

Two grids of tiles, twelve numbers, neither telling you anything the screen was
not already showing. Overview's six tiles were the sidebar's own count badges
restated; the timeline's four status tiles restated the bar colours the legend
already names. The status counts moved **into the status filter**, where the
level filter had been carrying its counts all along — a count you can act on
beats one you can only read. On a 1500×1200 window the plan now starts at 420px
instead of 700.

`POST /api/todos/parse` turns one free-text line into the create form's fields
through the same grammar-constrained `structured()` choke point every other LLM
call uses. It **fills** the dialog and never creates anything: the difference
between a wrong draft you can see and a wrong row already in the database is
the whole design. Everything the model returns is treated as untrusted — an
invented priority falls back to medium, a malformed or impossible date is
dropped, title and owner are redacted and truncated rather than rejected. An
unreachable model is a 503 naming the cause and the fix, and the dialog stays
usable.

### 0.8.0 — every task is a bar

A task with a deadline and no planned start was a 1px tick: it said *when*
something was due and nothing about how much time was left to do it, and a lane
of ticks is a list with extra steps. Only a milestone stays a diamond, because
a gate is an instant, not work.

What the bar spans is the **runway** — today to the deadline, or the deadline to
today once it is overrun. That is a real quantity and it grows every day the
item stays open. It is also not a schedule anybody wrote, so a deadline-only
bar is drawn open at the left and capped at the deadline. A finished item gets
no runway; extending it to today would draw work that stopped weeks ago as
still running.

### 0.8.1 — the delete button that could not deliver

The list on Planung merges two stores and put the same delete button on every
row. A schedule item's id has never been in the task database, so
`DELETE /api/tasks/<id>` did exactly what it should and answered 404. The
client now routes by **row kind** rather than by guessing at the object, and
plan items became deletable —
`DELETE /api/schedule/{project_id}/items/{item_id}` with
`POST /api/schedule/restore` behind the same Undo toast. The item's index is
part of the undo token (re-appending would silently reorder a plan), restore is
idempotent, and the file is rewritten through a temp file and `os.replace`.

### 0.9.0 — the Copilot POC page

A temporary sidebar entry gathering everything about the Microsoft 365
interface, so judging it does not mean reading source. It leads with the
distinction that causes most of the confusion: **Prompt-Export** (no setup, any
tenant, works today — every number on the dashboard came from it) versus
**Graph API** (removes the copy-paste, needs an app registration, ships off).

`GET /api/m365/status` reports the five preconditions with the command that
fixes each; local facts only, because a documentation page that phones
Microsoft to render itself is slow when the tenant is unreachable and wrong on
a machine with no network. `POST /api/m365/probe` makes one real Retrieval
call, signing in from the token cache only — a device code blocks until
somebody types it on another device.

## 9.3 What is deliberately still open

- **The M365 POC is unverified against a live tenant.** Every test stubs the
  HTTP layer; the failure paths are exercised end to end, the happy path is
  not. The connection test on the Copilot POC page is what answers this, and it
  needs a signed-in account.
- **Editing is the largest product gap.** Task status and schedule owner are
  the only fields editable anywhere. Titles, dates and priorities can be
  created and deleted but not changed.
- **`pytest-playwright` is not in the `[dev]` extras**, so the browser suite
  skips silently on a fresh install. A run reporting "skipped" is not green.
- **The Copilot POC page is temporary** by construction. When the POC is
  adopted or dropped, the screen, its key, its nav entry and its CSS block come
  out together.
