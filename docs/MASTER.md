...
> **⚠ Superseded — this is not the project document.**
> The live document is **`HERMES_Local_Assistant_COMPLETE.html`** in the
> repository root: spec, POC runbook and the delivered-release history in
> one self-contained file. This Markdown is kept for history only. Do not
> extend it — anything written here will not be read.
...
*End of the superseded Markdown. The live document is `HERMES_Local_Assistant_COMPLETE.html`; extend the relevant section there.*
